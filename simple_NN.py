...
simple_Dx = Dropout(0.3)(simple_Dx)
for layercount in range(int(sys.argv[2])):
    ...
    simple_Dx = Dropout(0.1)(simple_Dx)
...
simple_D.compile(loss="binary_crossentropy", optimizer=SGD(lr=0.02, momentum=0.2))
simple_D.summary()
...
def plot_simple_losses(losses1, losses2, counter):
    ...
    plt.grid()
    plt.gcf().savefig(output_path + 'simple_NN_losses' + file_extension + '_' + str(counter) + '.png')
    plt.gcf().clear()
    plt.show()
...
for i in range(int(Options['SimpleTrainEpochs'])):
    simple_D.fit(train_event_transfered, train_target, batch_size=None, sample_weight=train_weight, steps_per_epoch = 20, validation_steps = 20, verbose = 1, epochs=1)
    ...
    if i % 100 == 0:
        plot_simple_losses(lossessimpletest, lossessimpletrain, i)
# ## 3.3 Test
