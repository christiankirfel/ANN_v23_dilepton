...
file_extension = sys.argv[1] + '_' + str(int(sys.argv[2]) +1) + '_' + sys.argv[3] + '_wt_sys'
...
#     display.display(plt.gcf())
    ...
    plt.legend(loc="upper right")
    plt.grid()
    plt.gcf().savefig(output_path + 'ANN_D_Pretrain_ROC_' + file_extension + '.png')
    #plt.show()
    plt.gcf().clear()
...
for i in range(int(Options['PreTrainEpochs'])):
    ANN_D.fit(train_event_transfered, train_target, sample_weight=train_weight, epochs=1)
    ...
    plot_D_losses(lossesDtest, lossesDtrain, lossesRtest, lossesRtrain)
...
print "The simple network used " + str(int(sys.argv[2])+1) + " layers with " + sys.argv[3] + " nodes each"
print "The D network used " + str(int(sys.argv[4])+1) + " layers with " + sys.argv[5] + " nodes each"
print "The R network used " + str(int(sys.argv[6])+1) + " layers with " + sys.argv[7] + " nodes each"
